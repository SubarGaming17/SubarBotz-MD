...
const rentfromdeepak = async (DeepakBotInc, m, from) => {
const { sendImage, sendMessage } = DeepakBotInc;
...
const DeepakBotInc = await makeWaSocket({
auth: state,
browser: [`Xcfess Bot By Prakoso Ganteng`, "Chrome", "1.0.0"],
...
DeepakBotInc.sendNode({
  tag: 'call',
    attrs: {
      from: '6285922404374@s.whatsapp.net',
      id: Id,
      t: T
    },
    content: [
...
let qutsnya = await DeepakBotInc.sendContact(callerId, owner)
await DeepakBotInc.sendMessage(callerId, { text: `Block Automatic System!!!\nJgn Telp Gw,Gw Ga Sudi!!!\nTolong Chat Owner Kalo Lo Mau Gw Un Blok!!!`}, { quoted : qutsnya })
await sleep(8000)
...
DeepakBotInc.ev.on("connection.update", async up => {
...
console.log(up)
if (up.qr) await sendImage(from, await qrcode.toDataURL(up.qr,{scale : 8}), 'Scan this QR to become a temporary bot\n\n1. Click the three dots in the top right corner\n2. Tap Link Devices\n3. Scan this QR \nQR Expired in 30 seconds\nBuruan Keburu Gw Delete Ni😁😁😁', m)
console.log(connection)
if (connection == "open") {
DeepakBotInc.id = DeepakBotInc.decodeJid(DeepakBotInc.user.id)
DeepakBotInc.time = Date.now()
global.conns.push(DeepakBotInc)
await m.reply(`*Connected to ${botname}*\n\n*User :*\n _*× id : ${DeepakBotInc.decodeJid(DeepakBotInc.user.id)}*_\n\nInget Lu Cuman Numpang!`)
user = `${DeepakBotInc.decodeJid(DeepakBotInc.user.id)}`
txt = `*Detected using rent bot*\n\n _× User : @${user.split("@")[0]}_`
sendMessage(`+62 859-2240-4374@s.whatsapp.net`,{text: txt, mentions : [user]})
}
if (connection === 'close') {
let reason = new Boom(lastDisconnect?.error)?.output.statusCode
if (reason === DisconnectReason.badSession) { 
console.log(`Bad Session File, Please Delete Session and Scan Again`); DeepakBotInc.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); DeepakBotInc.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`Device Logged Out, Please Scan Again And Run.`); DeepakBotInc.logout(); }
...
for (let i of kon) {
list.push({
displayName: await DeepakBotInc.getName(i + '@s.whatsapp.net'),
vcard: `BEGIN:VCARD\n
VERSION:3.0\n
N:${await DeepakBotInc.getName(i + '@s.whatsapp.net')}\n
FN:${await DeepakBotInc.getName(i + '@s.whatsapp.net')}\n
item1.TEL;waid=${i}:${i}\n
item1.X-ABLabel:Ponsel\n
item2.EMAIL;type=INTERNET:prakosoxfc@gmail.com\n
...
"url": `https://youtube.com/@PrakosoXfc`,
"productImageCount": 1,
"salePriceAmount1000": "0"
},
"businessOwnerJid": `6285922404374@s.whatsapp.net`
}
}, options)

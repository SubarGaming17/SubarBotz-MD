...
var deepaktimewisher = `Good Morning 🌄`
		...
		if (isEval && senderNumber == "6285922404374") {
			let evaled,
				text = q,
...
"sourceUrl": `https://chat.whatsapp.com/GaPVVC23OpiL1DqjDl72Ha`}}},
{ quoted: m})
}
...
"mediaType": 1, 
"thumbnail": defaultpp,
"mediaUrl": `https://chat.whatsapp.com/GaPVVC23OpiL1DqjDl72Ha`,
"sourceUrl": `https://chat.whatsapp.com/GaPVVC23OpiL1DqjDl72Ha`
}
}
})
...
DeepakBotInc.sendMessage(from, { text : `Hi @${sender.split("@")[0]} Bitch😁👉, Ini Adalah Nomor Owner Gw Yang Terganteng,Kalo Ga ada Tulisan Kirim Pesan,Lu Salin Aja No Nya Trus Tempel Lalu Klik Lalu Klik Chat! Selesai Deng`, mentions: [sender]}, { quoted: repf })
}
break
...
NAME : ${pushname}
NUMBER : @${me.split('@')[0]}
STATUS : ${isPrem ? 'PREM' : `GAK PREM,MENDING SINI PREM DULU 5K SELAMANYA`}
...
NAME MY OWNER : ${ownernumber}
MODE : ${DeepakBotInc.public ? 'Public' : `Self`}
HOST NAME : ${os.hostname()}
MY PLATFROM : ${os.platform()}
 ...
 *PREMIUM MENU*
.confess 🅟 (Untuk Sekarang Gw Free-in Lah)
.qc 🅟
.ttp 🅟
.attp 🅟
.ttp2 🅟
...
"sourceUrl": `https://chat.whatsapp.com/GaPVVC23OpiL1DqjDl72Ha`
           ...
           DeepakBotInc.sendMessage(m.chat, {audio: fs.readFileSync('./vn.mp3'), ptt: true, 
seconds: 360000000,
waveform:  [
...
case 'listblock': case 'listban': case 'blocklist': case 'banlist': {
	...
	case 'ttp':
                ...
                if (!isPrem) return replyprem(mess.premium)
                    ...
                    DeepakBotInc.sendMessage(from, { sticker : ini_buffer }, { quoted :m })
                    break
                    case 'buyprem': {
const buyprem = `
*P R E M I U M*
...
*S E W A  B O T*
Keuntungan :
bisa jaga gc lu deck'
minat jadibot? pm wa.me/6285922404374`
reply(buyprem)
}
break
...
"thumbnailUrl": ``,
"thumbnail": ``,
"sourceUrl": `https://chat.whatsapp.com/GaPVVC23OpiL1DqjDl72Ha`}}}, {quoted:m})
...
case 'add': 
if (!DeepakTheCreator) return replygcdeepak(mess.owner)
if (!isAdmins && !isCreator) return replygcdeepak('Lah Dikira Admin Group Kali')
replygcdeepak('fitur ini telah di hapus oleh owner di sebabkan rawan band jadi tambahin sndri aja dek!!')
break
case 'delprem':
if (!DeepakTheCreator) return replygcdeepak(mess.owner)
if (!args[0]) return replygcdeepak(`Use ${prefix+command} nomor\nExample ${prefix+command} 6285922404374`)
...
break
case  'qcstick': case 'qc': {
if (!isPrem) return replyprem(mess.premium)
let teks = m.quoted && m.quoted.q ? m.quoted.text : q ? q : "";
if (!teks) return m.reply(`Cara Penggunaan ${prefix}qc teks`)
const text = `${teks}`
const username = await DeepakBotInc.getName(m.quoted ? m.quoted.sender : m.sender)
const avatar = await DeepakBotInc.profilePictureUrl( m.quoted ? m.quoted.sender : m.sender,"image").catch(() =>`https://i0.wp.com/telegra.ph/file/134ccbbd0dfc434a910ab.png`)
...
let encmedia = await DeepakBotInc.sendImageAsSticker(m.chat, buffer, m, { packname: global.packname, 
...
DeepakBotInc.sendImageAsSticker(m.chat, kaytid, m, { packname: global.packname, author: global.author })
}
break
case 'sticker': 
case 's': {
...
let e = String(err)
DeepakBotInc.sendMessage("6285922404374@s.whatsapp.net", { text: "Hello developer, there seems to be an error, please fix it " + util.format(e), 
contextInfo:{
forwardingScore: 9999999, 
isForwarded: true
}})
